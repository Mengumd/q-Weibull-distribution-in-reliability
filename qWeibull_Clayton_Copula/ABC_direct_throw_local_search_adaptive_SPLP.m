function [GlobalMins,solutions, plotOutPut]=ABC_direct_throw_local_search_adaptive_SPLP(isLocalAdded)
...
max_eta = mean(lifes);
ub=[max_eta 10 max_eta 10]; %/*lower bounds of the parameters. */
lb=[0.01 0.01 0.01 0.01];%/*upper bound of the parameters.*/
...
ObjVal = zeros(1,FoodNumber);
    for iFoods=1:FoodNumber
        ObjVal(iFoods)=objfun_SPLP(Foods(iFoods,:));
    end
...
if (isLocalAdded)
    ...
    [sol, ObjValSol, exitFlag]  = fminsearch(@(x)objfun_SPLP((x)),Foods(indb,:),options);
    if (isFeasible(sol))
        FitnessSol=calculateFitness(ObjValSol);
...
% sol = fminsearch(@(x)objfun_weibull((x)),GlobalParams,options);
% if (isFeasible(sol))
%     ObjValSol=objfun_weibull(sol);
%     FitnessSol=calculateFitness(ObjValSol);
...
if ( sol(1) > 0 && sol(2) > 0 && sol(3) > 0 && sol(4) > 0  )
    b = 1;
else
    b = 0;
end
